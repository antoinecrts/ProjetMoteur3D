...
GPUVariableBase* GLUniformManager::getUniformBase(string name)
{
	...
	{
		LOG_WARNING  << m_Program->getName() <<":  There is no active uniform named " << name << std::endl;
		to_ret = NULL;
	}
...
GPUVariableBase* GLUniformManager::createGPUVariable( GLenum type,string name,GLint program,GLuint uBuffer,GLint offset,GLint loc,GLint size)
{
	...
	switch (type)
	{
		...
		default :
			LOG_WARNING << m_Program->getName() << " :    GPU type for variable " << name << " not yet implemented in Uniform Manager." << endl;
			v = NULL;
			break;
	}
...
GPUfloat* GLUniformManager::getGPUfloat(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a float. Returning dummy variable. "<<endl;
...
GPUvec2* GLUniformManager::getGPUvec2(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a vec2. Returning dummy variable. "<<endl;
		to_ret = &m_Dvec2;
	}
...
GPUvec3* GLUniformManager::getGPUvec3(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a vec3. Returning dummy variable. "<<endl;
		to_ret = &m_Dvec3;
	}
...
GPUvec4* GLUniformManager::getGPUvec4(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a vec4. Returning dummy variable. " << std::endl;
		to_ret = &m_Dvec4;
	}
...
GPUdouble* GLUniformManager::getGPUdouble(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a double. Returning dummy variable. "<<endl;
		to_ret = &m_Ddouble;
	}
...
GPUdvec2* GLUniformManager::getGPUdvec2(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a dvec2. Returning dummy variable. "<<endl;
		to_ret = &m_Ddvec2;
	}
...
GPUdvec3* GLUniformManager::getGPUdvec3(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a dvec3. Returning dummy variable. "<<endl;
		to_ret = &m_Ddvec3;
	}
...
GPUdvec4* GLUniformManager::getGPUdvec4(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a dvec4. Returning dummy variable. "<<endl;
		to_ret = &m_Ddvec4;
	}
...
GPUint* GLUniformManager::getGPUint(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a int. Returning dummy variable. "<<endl;
		to_ret = &m_Dint;
	}
...
GPUivec2* GLUniformManager::getGPUivec2(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a ivec2. Returning dummy variable. "<<endl;
		to_ret = &m_Divec2;
	}
...
GPUivec3* GLUniformManager::getGPUivec3(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a ivec3. Returning dummy variable. "<<endl;
		to_ret = &m_Divec3;
	}
...
GPUivec4* GLUniformManager::getGPUivec4(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a ivec4. Returning dummy variable. "<<endl;
		to_ret = &m_Divec4;
	}
...
GPUmat2* GLUniformManager::getGPUmat2(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a mat2. Returning dummy variable. "<<endl;
		to_ret = &m_Dmat2;
	}
...
GPUmat3* GLUniformManager::getGPUmat3(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a mat3. Returning dummy variable. "<<endl;
		to_ret = &m_Dmat3;
	}
...
GPUmat4* GLUniformManager::getGPUmat4(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a mat4. Returning dummy variable. "<<endl;
		to_ret = &m_Dmat4;
	}
...
GPUdmat2* GLUniformManager::getGPUdmat2(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a dmat2. Returning dummy variable. "<<endl;
		to_ret = &m_Ddmat2;
	}
...
GPUdmat3* GLUniformManager::getGPUdmat3(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a dmat3. Returning dummy variable. "<<endl;
		to_ret = &m_Ddmat3;
	}
...
GPUdmat4* GLUniformManager::getGPUdmat4(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a dmat4. Returning dummy variable. "<<endl;
		to_ret = &m_Ddmat4;
	}
...
GPUbool* GLUniformManager::getGPUbool(string name)
{
	...
	if (!to_ret)
	{
		LOG_WARNING << m_Program->getName() << " :    Variable "<< name << " is not a bool. Returning dummy variable. "<<endl;
		to_ret = &m_Dbool;
	}
...
GPUsampler* GLUniformManager::getGPUsampler(string name)
{
	...
	{
		LOG_WARNING << m_Program->getName() << " :    There is no active sampler named " << name << ". Returning dummy sampler. " << endl;
		to_ret = &m_Dsampler;
	}
