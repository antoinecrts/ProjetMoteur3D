...
void GLProgramsSourceManager::addFile(string f_path,string name,string cName)
{
	...
	if (ext != string::npos)
	{
		...
		if (typ == string::npos)
		{
			...
			if (m_listNamedStrings.find(finalName) == m_listNamedStrings.end()) // Search Existing list
			{
				LOG_TRACE << "Loading : " << finalName << std::endl;
		...
		else
		{
			...
			if (!type.compare("VS"))
				...
				LOG_ERROR << "unkown shader type for file " <<  f_path+name << endl;
		}
...
GLProgramSource* GLProgramsSourceManager::get(string name,GLenum type)
{
	...
	else
	{
		...
		LOG_TRACE << " WARNING :: " + typeString + " program " + name + " is unknown : using default stage\n" << std::endl;
