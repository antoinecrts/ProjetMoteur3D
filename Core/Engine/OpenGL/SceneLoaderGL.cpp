...
SceneLoaderGL::SceneLoaderGL(std::string BasePath)
{
	m_basePath = BasePath;
}
...
Node* SceneLoaderGL::loadScene(string filename, bool SmoothNormal)
{
	...
	LOG_INFO << "Loading Scene File : " << filename << std::endl;

	if (m_basePath.size() == 0)
	{
		
		std::size_t found = filename.find_last_of("/\\");
		m_basePath = filename.substr(0, found);
	}
	LOG_INFO << "Base Path : " << m_basePath << std::endl;
	...
	if (!scene)
	{
		LOG_WARNING << string(importer.GetErrorString()) << std::endl;
...
LightNode* SceneLoaderGL::createLightNode(aiNode* node, aiLight* light) // TODO : add support for more complex light
{
	...
	lnode->setModel(gobScene->getModel<ModelGL>(ressourceCoreObjPath + "Sphere.obj"));
...
void SceneLoaderGL::addDefaultLight(Node* targetnode)
{
	...
	LOG_INFO << "No Light in scene, adding default white light" << std::endl;
...
{
	LOG_TRACE << "Loading camera " << cam->mName.C_Str() << std::endl;
...
Model* SceneLoaderGL::parseModel(aiMesh* mesh)
{
	...
	modelGL->loadToGPU();
	gobScene->m_Models.insert(modelGL->getName(), modelGL);
...
ColorMaterial* rtmat = new ColorMaterial("Color Material",
