...
{
	info_text = "";
	src = prgMgr.get(name,type); //permet de récupérer le GLProgram correspondant au nom et au type donnés
	if (src) m_UniformMgr = new GLUniformManager(this); //création d'un nouveau GLUniformManager afin de pouvoir utiliser des variables uniforms
}

GLProgram::~GLProgram()
{
	if (src != NULL) prgMgr.release(m_Name,m_Type); //libère le GLProgram
