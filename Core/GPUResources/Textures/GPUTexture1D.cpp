...
bool GPUTexture1D::create(std::string filename)
{
	...
	if (m_TextureId == 0)
	{
		LOG_ERROR << "ERROR : GPUTexture1D - SOIL loading error in file " << filename << " : " << SOIL_last_result() << std::endl;
		return false;
	}
...
bool GPUTexture1D::create(unsigned char *data, int width, int channels)  
{
	...
	if (m_TextureId == 0)
	{
		LOG_ERROR << "ERROR : GPUTexture1D - loading from memory error : " << m_Name << " : " << SOIL_last_result() << std::endl;
		return false;
	}
	glGenerateMipmap(m_Target);
	setUpSampler(GL_REPEAT,GL_LINEAR_MIPMAP_LINEAR);
...
bool GPUTexture1D::create(float *data, int width, int channels)
{
	...
	if (channels == 4)
	{
		...
		glTexSubImage1D(GL_TEXTURE_1D, 0, 0, width, GL_RGBA, GL_FLOAT, data);
	}
