...
bool GPUTexture2DArray::addLayer(std::string filename, int layer,bool force, const glm::vec4 & defValue )
{
	...
	{
		LOG_WARNING << " GPUTexture2DArray - file " << filename << " not found. Using default value texture instead" << std::endl;
		width = m_Width;
		height = m_Height;
		channels = 4;
	...
	{
		LOG_ERROR  << " GPUTexture2DArray - SOIL loading error in file " << filename << " : " << SOIL_last_result() << std::endl;
		return(false);	
	}
